...
app.get("/", async (req, res) => {
  ...
  if (!req.session.user) {
    ...
    else (req.session.loginType == "consumer")
...
function buildFuzzySearchCondition(search, columnName) {
  ...
  for (let word of words) {
    ...
    conditions.push(`SOUNDEX(${columnName}) = SOUNDEX(?)`);
    params.push(word);
  }
  return {
...
app.get("/consumerpage", async (req, res) => {
  try {
    if (!req.session.isAuthenticated) {
      req.session.msg = {
        type: "error",
    ...
    if (req.session.loginType !== "consumer") {
      req.session.msg = {
        type: "error",
        text: "This page is only accessible to consumer users"
      };
      return res.redirect("/");
    }
    ...
    if(search && search.trim() !== ""){
      const fuzzy = buildFuzzySearchCondition(search, "p.title");

      countQuery += fuzzy.condition;
      productQuery += fuzzy.condition;

      countParams.push(...fuzzy.params);
      productParams.push(...fuzzy.params);
    }
    ...
    productQuery += ` ORDER BY CASE WHEN m.district = ? THEN 0 ELSE 1 END LIMIT ? OFFSET ?`

    productParams.push(userDistrict, limit, offset);
    ...
    console.error("Consumerpage error:", error);
    res.status(500).send("Something went wrong. Check terminal.");
  }
});
...
app.post("/addOne", upload.single("photo"),
  ...
  async (req, res) => {
    try {
      ...
      await db.query("INSERT INTO products (img, title, stock, normal_price, discount_price, expire_date, email,city,district) VALUES (?,?,?,?,?,?,?,?,?)",
        [req.file.filename, form.title, form.stock, form.normal, form.discount, form.date, req.session.user.email, req.session.user.city,req.session.user.district])
